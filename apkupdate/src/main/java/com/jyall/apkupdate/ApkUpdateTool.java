...
public class ApkUpdateTool {
    ...
    public ApkUpdateTool(Context mContext, OnUpdateListener onUpdateListener) {
        this.onUpdateListener = onUpdateListener;
        this.sharedPreferences = mContext.getSharedPreferences("apkUpdate", Context.MODE_PRIVATE);
     ...
     * @param updateInfo
     */
    public void updateVersion(Context context,UpdateInfo updateInfo,boolean isShowToast) {
        //正在下载过程中就不用再显示更新的弹窗了
        ...
        if (!TextUtils.isEmpty(updateInfo.version)) {
            if (Integer.parseInt(updateInfo.version) > getPackageInfo(context).versionCode) {
                initDialog();
            } else {
                if(isShowToast){
