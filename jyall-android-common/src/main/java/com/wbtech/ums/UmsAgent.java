     ...
     static void onError(final Context context, final String errorinfo) {
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                em.postErrorInfo(errorinfo,context);
            }
        });
        handler.post(thread);
    }
...
//                String topath= Environment.getExternalStorageDirectory()+"/"+"jyallUM"+"/";
//                SimpleDateFormat    formatter    =   new SimpleDateFormat("yyyy年MM月dd日   HH:mm:ss  ");
//                Date    curDate    =   new Date(System.currentTimeMillis());//获取当前时间
//                String    str    =    formatter.format(curDate);
//                String  tag="event_id--"+event_id+"label--"+label+"acc--"+acc+"time--"+str;
//              int s=  CopyFile.WriteSdcardFile(tag,topath,"jyalljsjw");

                em.postEventInfo(context);
            }
        });
        handler.post(thread);
    }
   ...
   public static void postPushID(final Context context, final String cid){
    ...
    public static void postWebPage(final Context context,final String pageName){
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (usinglogManager == null)
                    usinglogManager = new UsinglogManager(context);
                usinglogManager.onWebPage(pageName,context);
            }
        });
        handler.post(thread);
        }
