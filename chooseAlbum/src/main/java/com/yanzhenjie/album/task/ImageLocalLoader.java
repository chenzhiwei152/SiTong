...
public class ImageLocalLoader {
    ...
    public static Drawable DEFAULT_DRAWABLE = new ColorDrawable(Color.parseColor("#FF2B2B2B"));
    ...
    public Bitmap readImage(String imagePath, int maxWidth, int maxHeight) {
        ...
        boolean isExist = imageFile.exists();
        if (isExist) {
    ...
    private Bitmap getImageFromCache(String key, int width, int height) {
        String cachKey = key + width + height;
        return mLruCache.get(cachKey);
    }
