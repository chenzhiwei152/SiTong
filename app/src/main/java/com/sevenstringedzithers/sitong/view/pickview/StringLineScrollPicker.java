package com.sevenstringedzithers.sitong.view.pickview;
 ...
 */
public class StringLineScrollPicker extends ScrollPickerView<CharSequence> {
    ...
    public StringLineScrollPicker(Context context, AttributeSet attrs) {
    ...
    private void computeColor(int relative, int itemSize, float moveLength) {
        ...
        if (relative == -1 || relative == 1) { // 上一个或下一个
            ...
            if ((relative == -1 && moveLength < 0)
                ...
                color = mEndColor;
//                color = getResources().getColor(R.color.color_99d0a670);
            } else { // 计算渐变的颜色
